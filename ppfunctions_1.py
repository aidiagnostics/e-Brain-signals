...
def get_Spectrogram(audio, Fs, fft_size = 200, step_size = 1, spec_thresh = 2.5, window_type=0):
    ...
    # fft_size = 200 # 1000 samples represent 500ms in time  # window size for the FFT
    # step_size = int(fft_size/200) # distance to slide along the window (in time) if devided by 40 is good
    # spec_thresh = 2.5 # threshold for spectrograms (lower filters out more noise)
    return abs(move_spec(aid_spectrogram(audio.astype('float64'), fft_size = fft_size, window_type=0, step_size = step_size, log = True, thresh = spec_thresh)))/2.5
...
def hs_longVector(h_sounds, h_labels, pc=0.70):
    '''
    Heart Sound Long Vectors
    input: 
        h_sounds: Heart sound recordings in a list (NxS) where N-> no. recordings; S-> no. samples per recording.
    ...
    for i in range(len(h_sounds)):
        tr = int(len(h_sounds[i])*pc)     # number of samples per sound FOR TRAINING
        
        train_s.append(h_sounds[i][:tr])  # sounds for training
        train_l.append(h_labels[i][:tr])  # labels for training
        test_s.append(h_sounds[i][tr:])   # sounds for testing
        test_l.append(h_labels[i][tr:])   # labels for testing
    # Flatten the vectors    
    train_vectorS = np.array(list(flatten(train_s)))
    train_vectorL = np.array(list(flatten(train_l)))
    test_vectorS = np.array(list(flatten(test_s)))
    test_vectorL = np.array(list(flatten(test_l)))
    
    return train_vectorS, train_vectorL, test_vectorS, test_vectorL

def nn_org_dataset(long_hs_vector, long_lb_vector, tc=0.1, fs=2000):
    '''
    organise dataset for NN
    input: 
        long_hs_vector: 1D array containing the whole recordings sticked to each other
    ...
    ex = int(len(long_hs_vector)/chunks) # Number of examples per vector
    mat_s = np.zeros([ex,chunks])   # Sound matrix
...
# Organizing label matrix
def sq_labels(X):
    label_matrix = np.zeros([len(X), 6])
    for i in tqdm(range(len(X))):
        ...
        if round(np.mean(X[i,:])) == 2.0:
            label_matrix[i,2] = 1
        if round(np.mean(X[i,:])) == 3.0:
            label_matrix[i,3] = 1
        if round(np.mean(X[i,:])) == 5.0:
            label_matrix[i,4] = 1
        if round(np.mean(X[i,:])) == 6.0:
            label_matrix[i,5] = 1
    return label_matrix

def sq_labels_4(X):
    label_matrix = np.zeros([len(X), 4])
    for i in tqdm(range(len(X))):
        if round(np.mean(X[i,:])) == 0.0:
            label_matrix[i,0] = 1
        if round(np.mean(X[i,:])) == 1.0:
...
def change_labels(x):
    ''' This functions change the numbers of a label vectors to another specifies numbers
    Here we changed the label vector in order to get: 0=S1, 1=S2, 3=murmur, 2=others'''
    z = []    
    for i in tqdm(range(len(x))):
        y = np.zeros(len(x[i]))
...
def audio_to_chunks(audio_vector, tc=0.1, fs=2000):
    '''
    organise dataset for NN
    input: 
    ...
    '''
    chunks = int(fs*tc) # number of samples in a time 't' at 2kHz sampling freq.
    ex = int(len(audio_vector)/chunks) # Number of examples per vector
    mat_s = np.zeros([ex,chunks])   # Sound matrix
#    mat_l = np.zeros([ex,chunks])   # Label matrix
    for i in range(ex):
        mat_s[i,:] = audio_vector[i*chunks:(1+i)*chunks]
#        mat_l[i,:] = long_lb_vector[i*chunks:(1+i)*chunks]
    return mat_s#, mat_l
